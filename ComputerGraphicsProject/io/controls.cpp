...
glm::vec3 io::GetPosition() {
	return position;
}

glm::vec3 io::GetLightPosition() {
	return lightPos;
}
...
void io::ComputeMatricesFromInputs() {
	...
	if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS) {
		lightPos += glm::vec3(deltaTime * speed, 0, 0);
	}
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
		lightPos -= glm::vec3(deltaTime * speed, 0, 0);
	}
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
		lightPos += glm::vec3(0, deltaTime * speed, 0);
	}
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) {
		lightPos -= glm::vec3(0, deltaTime * speed, 0);
	}
	if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS) {
		lightPos += glm::vec3(0, 0, deltaTime * speed);
	}
	...
	ProjectionMatrix = glm::perspective(glm::radians(FoV), 1280 / 720.0f, 0.1f, 100.0f);
	// Camera matrix
